...
void main(){
   ...
   // OC1R = 19999;              // initialize before turning OC1 on; afterward it is read-only
    ...
    while(message[index]){
        ...
        int var = message[index];
        int xloc = startX + (5*index); //increment X position

        //if(xloc>= (maxX-5)) { //if end of line is reached, RETURN
         //   int yloc = yloc + 5; //go to next row
         //   int xloc = startX; //return to starting position on the left

        //}

        //if(var==0) cyclestring = 0; //exit loop if '/0' is reached
